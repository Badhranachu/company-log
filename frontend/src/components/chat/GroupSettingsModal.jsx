...
import { FiArrowLeft, FiX, FiUserCheck, FiMoreVertical, FiUserMinus } from 'react-icons/fi'
...
export default function GroupSettingsModal({ open, onClose, chat, canManage, mediaOnly = false }) {
  ...
  const doAddMember = async (uid) => {
    ...
    try {
      ...
      if (result?.already_member) {
        toast('Already a member of this group', { icon: 'i' })
      } else {
        ...
        <motion.form initial={{ y: 16, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="w-full max-w-xl max-h-[90vh] overflow-y-auto rounded-2xl bg-white dark:bg-slate-900 p-5 space-y-4" onSubmit={save}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-xl font-semibold">{mediaOnly ? 'Media' : 'Group Settings'}</h3>
            </div>
                ...
                <label className="text-xs rounded-xl border px-3 py-2 cursor-pointer">
                  ...
                  <input type="file" className="hidden" accept="image/*" disabled={!canManage}
                    onChange={async (e) => {
                      ...
                      const f = raw ? await compressImage(raw, { maxWidth: 400, maxHeight: 400, quality: 0.85 }) : null
                      setForm({ ...form, group_avatar: f })
                      setAvatarPreview(f ? URL.createObjectURL(f) : (chat?.group_avatar_url || ''))
                    }} />
                </label>
                <label className="text-xs rounded-xl border px-3 py-2 cursor-pointer">
                  Group banner
                  <input type="file" className="hidden" accept="image/*" disabled={!canManage}
                    onChange={async (e) => {
                      const raw = e.target.files?.[0] || null
                      const f = raw ? await compressImage(raw, { maxWidth: 1200, maxHeight: 400, quality: 0.82 }) : null
                      setForm({ ...form, group_banner: f })
                      setBannerPreview(f ? URL.createObjectURL(f) : (chat?.group_banner_url || ''))
                    }} />
                </label>
              </div>
              <div className="grid grid-cols-2 gap-2">
              ...
              {canManage && (
                <div className="rounded-xl border p-3 space-y-2">
                  ...
                  <div className="max-h-36 overflow-auto space-y-1">
                    {users.filter(u => u.id !== currentUser?.id).length === 0 && userQuery && <p className="text-xs opacity-60 py-2 text-center">No users found</p>}
          ...
          <div className="rounded-xl border p-3 space-y-2">
            <p className="text-sm font-medium">{mediaOnly ? 'Media' : 'Group media'}</p>
          ...
          <div className="flex justify-end gap-2">
            ...
            {!mediaOnly && <button disabled={!canManage} className="px-3 py-2 rounded-xl bg-wa-600 text-white disabled:opacity-40">Save</button>}
          </div>
        </motion.form>
      </motion.div>
    </AnimatePresence>
